...
physeq <- readRDS("asv_table_cp_20240625.rds")

###View the OTU table)
### Convert to dataframe
df <- psmelt(physeq$Amplicon)
# df  <- df %>%
#   select(-OTU)
### View the dataframe
head(df)
...
```{r}
metadata <-  read.table("ready_metadata.tsv",
                          header           = TRUE,
                          sep              = "\t", 
                          stringsAsFactors = FALSE)
...
metadata$Quartile <-
  factor(metadata$Quartile, levels = c( "Quartile_1", "Quartile_2", "Quartile_3", "Quartile_4" ))
